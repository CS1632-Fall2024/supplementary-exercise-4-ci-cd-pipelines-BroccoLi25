...
public class RentACatUnitTest {
	...
	public void testGetCatNullNumCats0() {
		...
		try {
			...
			// invoke the getCat method w/ ID 2
			Cat result = (Cat) method.invoke(r, 2);

			// verify the result is null (no cats are present)
			assertNull(result);
	...
	public void testGetCatNumCats3() {
		...
		try {
			...
			// execution steps
			java.lang.reflect.Method method = r.getClass().getDeclaredMethod("getCat", RentACatImpl.class);
			method.setAccessible(true);
			when(c2.getId()).thenReturn(2);
			when(c2.getName()).thenReturn("Old Deuteronomy");
	...
	public void testRenameNumCat3() {
		...
		//when(c2.getId()).thenReturn(2);
		r.renameCat(2, "Garfield"); // using RentACat func
		Mockito.verify(c2).renameCat("Garfield"); // verify that renameCat("Garfield") was called for c2 b/c c2 is a mock
	}
